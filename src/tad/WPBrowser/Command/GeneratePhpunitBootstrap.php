...
class GeneratePhpunitBootstrap extends Command
{
	...
	{
		$this->setDefinition([
			new InputArgument('suites', InputArgument::OPTIONAL,
				'A comma separated list of suites PHPUnit tests should run.', 'functional'),
...
XML;
		...
		return $this->compileTemplate([
			'testSuites' => implode("\n", $suitesEntries),
			'bootstrapPath' => $this->bootstrapFilePath($testsPath)
		], $template);
	}
...
PHP;
		...
		$args['autoloadFile'] = sprintf("%s/%s/autoload.php", $relativeBackHopsPath,
			Utils::unleadslashit(Utils::untrailslashit($vendor)));
		$args['wpBrowserAutoloadFile'] = sprintf("%s/%s/lucatume/wp-browser/autoload.php", $relativeBackHopsPath,
			Utils::unleadslashit(Utils::untrailslashit($vendor)));
